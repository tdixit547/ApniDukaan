...
// Helper: resolve user display name across firstName/lastName or name fields
...
function displayProducts(products) {
    ...
    productGrid.innerHTML = products.map(product => {
        ...
        // Use imageUrl from product if available, otherwise show placeholder text
        const safeName = product.name.replace(/'/g, "\\'");
        const imageHtml = product.imageUrl
            ? `<img src="${product.imageUrl}" alt="${product.name}" style="width:100%;height:100%;object-fit:cover;" onerror="this.parentElement.innerHTML='<span style=\'font-size:13px;padding:8px;text-align:center;display:block;\'>${safeName}</span>';" />`
            : `<span style="font-size:13px;padding:8px;text-align:center;display:block;">${product.name}</span>`;
    ...
    try {
        ...
        modal.innerHTML = `
            <div class="modal-content product-details-modal">
                ...
                <p><strong>Stock:</strong> ${product.stock ?? product.stockQuantity ?? 0} units</p>
                ...
                <div style="margin-top: 20px;">
                    <button class="btn-primary" onclick="addToCart(${productId}); closeModal();" ${(product.stock ?? product.stockQuantity ?? 0) <= 0 ? 'disabled' : ''}>
                        Add to Cart
                    </button>
                </div>
            </div>
        ...
        modal.innerHTML = `
            <div class="modal-content quick-view-modal">
                ...
                `<span style="text-decoration: line-through; font-size: 18px; color: var(--text-muted); margin-left: 10px;">₹${product.basePrice.toFixed(2)}</span>` : ''}
                        ...
                        <p><strong>Stock:</strong> ${product.stock ?? product.stockQuantity ?? 0} units available</p>
...
// Show login modal (Deprecated - now handled by static auth screen)
function showLoginModal() {
    toggleAuthMode('login');
}

// Show register modal (Deprecated - now handled by static auth screen)
function showRegisterModal() {
    toggleAuthMode('register');
}

// Toggle between Login and Register in the auth screen
...
// Initialize static form listeners
document.addEventListener('DOMContentLoaded', function () {
    const loginForm = document.getElementById('staticLoginForm');
    if (loginForm) loginForm.addEventListener('submit', handleStaticLogin);

    const registerForm = document.getElementById('staticRegisterForm');
    if (registerForm) registerForm.addEventListener('submit', handleStaticRegister);
});

// Handle login from static form
async function handleStaticLogin(e) {
    e.preventDefault();

    const email = document.getElementById('staticLoginEmail').value;
    const password = document.getElementById('staticLoginPassword').value;
...
// Handle register from static form
async function handleStaticRegister(e) {
    e.preventDefault();

    const name = document.getElementById('staticRegName').value;
    const email = document.getElementById('staticRegEmail').value;
    const password = document.getElementById('staticRegPassword').value;
    const phoneNumber = document.getElementById('staticRegPhone').value;
    const address = document.getElementById('staticRegAddress').value;
...
// Close modal (Kept for compatibility with other modals like checkout)
function closeModal() {
